...
public class OpenAM {
	...
	private static List<Integer> RETRYABLE_500_HTTP_STATUS_CODES = Arrays.asList(HttpStatus.SC_SERVICE_UNAVAILABLE,
			HttpStatus.SC_INTERNAL_SERVER_ERROR, HttpStatus.SC_BAD_GATEWAY, HttpStatus.SC_GATEWAY_TIMEOUT);
	...
	private static CloseableHttpClient httpClientWithServiceUnavailableRetryStrategy() {
		final HttpClientBuilder httpClientBuilder = HttpClients.custom()
				.setServiceUnavailableRetryStrategy(new ServiceUnavailableRetryStrategy() {
					...
					public boolean retryRequest(final HttpResponse response, final int executionCount,
						...
						final boolean shouldRetry = ((statusCode == 429)
								|| OpenAM.RETRYABLE_500_HTTP_STATUS_CODES.contains(statusCode))
								&& (executionCount <= OpenAM.MAX_RETRIES);
						if (!shouldRetry)
							throw new RuntimeException(String.format("Not retrying %s. Count %d, Max %d",
									currentReq.getURI(), executionCount, OpenAM.MAX_RETRIES));

						OpenAM.logger.error("Retrying request on response status {}. Count {} Max is {}", statusCode,
								executionCount, OpenAM.MAX_RETRIES);
						return true;
					}
				});
		...
		httpClientBuilder.setRetryHandler((exception, executionCount, context) -> {
			final HttpUriRequest currentReq = (HttpUriRequest) context.getAttribute(HttpCoreContext.HTTP_REQUEST);
			Uninterruptibles.sleepUninterruptibly(OpenAM.RETRY_INTERVAL, TimeUnit.MILLISECONDS);
			OpenAM.logger.info("Encountered network error. Retrying request {},  Count {} Max is {}",
					currentReq.getURI(), executionCount, OpenAM.MAX_RETRIES);
			return executionCount <= OpenAM.MAX_RETRIES;
		});

		httpClientBuilder
				.setDefaultRequestConfig(RequestConfig.custom().setConnectionRequestTimeout(OpenAM.TIMEOUT_MILLIS)
						.setConnectTimeout(OpenAM.TIMEOUT_MILLIS).setSocketTimeout(OpenAM.TIMEOUT_MILLIS).build());
	...
	boolean fetch() throws IOException, ClientProtocolException, StreamWriteException, DatabindException {
		return this.fetchSaml2() && this.fetchWs() && this.fetchOAuth2() && this.fetchPolicies()
				&& this.fetchCircleOfTrust();
	}

	private boolean fetchCircleOfTrust()
			throws IOException, ClientProtocolException, StreamWriteException, DatabindException {
		final var mapper = new ObjectMapper();
		final var fetchCircleOfTrustRequest = new HttpGet(Configuration.getInstance().getOpenamUrl()
				+ "/json/realm-config/federation/circlesoftrust?_queryFilter=true");
		...
		OpenAM.logger.debug("fetchCircleOfTrustResponse: {}",
				fetchCircleOfTrustResponse.getStatusLine().getStatusCode());
	...
	private boolean fetchOAuth2() throws IOException, ClientProtocolException, StreamWriteException, DatabindException {
		final var mapper = new ObjectMapper();
		final var fetchOAuth2EntitiesRequest = new HttpGet(Configuration.getInstance().getOpenamUrl()
				+ "/json/realm-config/agents/OAuth2Client?_queryFilter=true");
		...
		OpenAM.logger.debug("fetchOAuth2EntitiesResponse: {}",
				fetchOAuth2EntitiesResponse.getStatusLine().getStatusCode());
		return fetchOAuth2EntitiesResponse.getStatusLine().getStatusCode() == 200;
	}

	private boolean fetchPolicies()
			throws IOException, ClientProtocolException, StreamWriteException, DatabindException {
		final var mapper = new ObjectMapper();
		final var fetchPoliciesRequest = new HttpGet(
				Configuration.getInstance().getOpenamUrl() + "/json/policies?_queryFilter=true");
	...
	private boolean fetchSaml2() throws IOException, ClientProtocolException, StreamWriteException, DatabindException {
		...
		final var mapper = new ObjectMapper();
		final var fetchSaml2EntitiesRequest = new HttpGet(Configuration.getInstance().getOpenamUrl()
				+ "/json/realm-config/federation/entityproviders/saml2?_queryFilter=true");
		...
		OpenAM.logger.debug("fetchSaml2EntitiesResponse: {}",
				fetchSaml2EntitiesResponse.getStatusLine().getStatusCode());
	...
	private boolean fetchWs() throws IOException, ClientProtocolException, StreamWriteException, DatabindException {
		final var mapper = new ObjectMapper();
		final var fetchWsEntitiesRequest = new HttpGet(Configuration.getInstance().getOpenamUrl()
				+ "/json/realm-config/federation/entityproviders/ws?_queryFilter=true");
	...
	public Set<CircleOfTrust> getCircleOfTrusts() {
		return Entity.getAllEntities().values().stream().filter(e -> e instanceof CircleOfTrust)
				.map(e -> (CircleOfTrust) e).collect(Collectors.toSet());
	}

	public Set<CircleOfTrust> getCircleOfTrusts2025() {
		return this.getCircleOfTrusts().stream().filter(e -> OpenAM.patternCirceOfTrust2025.matcher(e.getID()).find())
				.collect(Collectors.toSet());
	}

	public Set<Entity> getCircleOfTrusts2031() {
		return this.getCircleOfTrusts().stream().filter(e -> OpenAM.patternCirceOfTrust2031.matcher(e.getID()).find())
				.collect(Collectors.toSet());
	}

	public Set<Policy> getExternalMFAPolicies() {
		return this.getPolicies().stream().filter(e -> Policy.patternExternalMFAPolicies.matcher(e.getID()).find())
				.collect(Collectors.toSet());
	}

	public Set<Policy> getExternalMFAPoliciesApplicable(final EntityID id) {
		return this.getExternalMFAPolicies().stream().filter(p -> p.getResources().contains(id))
				.collect(Collectors.toSet());
	}

	public Set<Policy> getInternalCERTPolicies() {
		return this.getPolicies().stream().filter(e -> Policy.patternInternalCERTPolicies.matcher(e.getID()).find())
				.collect(Collectors.toSet());
	}

	public Set<Policy> getInternalCERTPoliciesApplicable(final EntityID id) {
		return this.getInternalCERTPolicies().stream().filter(p -> p.getResources().contains(id))
				.collect(Collectors.toSet());
	}

	public Set<Policy> getInternalMFAPolicies() {
		return this.getPolicies().stream().filter(e -> Policy.patternInternalMFAPolicies.matcher(e.getID()).find())
				.collect(Collectors.toSet());
	}

	public Set<Policy> getInternalMFAPoliciesApplicable(final EntityID id) {
		return this.getInternalMFAPolicies().stream().filter(p -> p.getResources().contains(id))
				.collect(Collectors.toSet());
	}

	public Set<Policy> getInternalOnlyPolicies() {
		return this.getPolicies().stream().filter(e -> Policy.patternInternalOnlyPolicies.matcher(e.getID()).find())
				.collect(Collectors.toSet());
	}

	public Set<Policy> getInternalOnlyPoliciesApplicable(final EntityID id) {
		return this.getInternalOnlyPolicies().stream().filter(p -> p.getResources().contains(id))
				.collect(Collectors.toSet());
	}
	...
	public Set<OAuth2Client> getOAuth2Clients() {
		return Entity.getAllEntities().values().stream().filter(e -> e instanceof OAuth2Client)
				.map(e -> (OAuth2Client) e).collect(Collectors.toSet());
	}

	public Set<Policy> getPolicies() {
		return Entity.getAllEntities().values().stream().filter(e -> e instanceof Policy).map(e -> (Policy) e)
				.collect(Collectors.toSet());
	}
	...
	public Set<Saml2> getSaml2Entities() {
		return Entity.getAllEntities().values().stream().filter(e -> e instanceof Saml2).map(e -> (Saml2) e)
				.collect(Collectors.toSet());
	}

	public Set<Saml2> getSaml2IdPEntities() {
		return this.getSaml2Entities().stream().filter(e -> e.hasAttribute(Entity.IDENTITY_PROVIDER))
				.collect(Collectors.toSet());
	}

	public Set<Saml2> getSaml2SpEntities() {
		return this.getSaml2Entities().stream().filter(e -> e.hasAttribute(Entity.SERVICE_PROVIDER))
				.collect(Collectors.toSet());
	}

	public Set<Wsfed> getWsfedEntities() {
		return Entity.getAllEntities().values().stream().filter(e -> e instanceof Wsfed).map(e -> (Wsfed) e)
				.collect(Collectors.toSet());
	}

	public Set<Wsfed> getWsfedIdPEntities() {
		return this.getWsfedEntities().stream().filter(e -> e.hasAttribute(Entity.IDENTITY_PROVIDER))
				.collect(Collectors.toSet());
	}

	public Set<Wsfed> getWsfedSpEntities() {
		return this.getWsfedEntities().stream().filter(e -> e.hasAttribute(Entity.SERVICE_PROVIDER))
				.collect(Collectors.toSet());
	}

	private boolean login() throws IOException, ClientProtocolException {

		final var loginRequest = new HttpPost(Configuration.getInstance().getOpenamUrl()
	...
	void process() throws ClientProtocolException, IOException {
		...
		final var jsonSaml2Entities = this.mapper.readValue(Paths.get("jsonSaml2Entities.json").toFile(),
				JsonNode.class);
		...
		final var jsonOAuth2Entities = this.mapper.readValue(Paths.get("jsonOAuth2Entities.json").toFile(),
				JsonNode.class);
		OAuth2Client.process(jsonOAuth2Entities);

		final var jsonCircleOfTrust = this.mapper.readValue(Paths.get("jsonCircleOfTrust.json").toFile(),
				JsonNode.class);
	...
	public void updateAuthAsPerPolicies(final Entity entity) {
		if (OpenAM.getInstance().getResourcesForInternalMFAPolicies().contains(entity)) {
			...
			final var remarks = MessageFormat.format("INTERNAL_AUTH: {0}, Policies: {1}",
					entity.getAttribute(Entity.INTERNAL_AUTH),
			...
			final var remarks = MessageFormat.format("INTERNAL_AUTH: {0}, Policies: None",
					entity.getAttribute(Entity.INTERNAL_AUTH));
			entity.addRemarks(remarks);
		}
		...
		if (OpenAM.getInstance().getResourcesForExternalMFAPolices().contains(entity)) {
			...
			final var remarks = MessageFormat.format("EXTERNAL_AUTH: {0}, Policies: {1}",
					entity.getAttribute(Entity.EXTERNAL_AUTH),
					Util.json(policies.stream().map(Policy::getID).toArray()));
			entity.addRemarks(remarks);
			...
			final var remarks = MessageFormat.format("EXTERNAL_AUTH: {0}, Policies: None",
					entity.getAttribute(Entity.EXTERNAL_AUTH));
			entity.addRemarks(remarks);
		}
	}
