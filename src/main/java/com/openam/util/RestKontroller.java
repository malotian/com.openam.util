...
public class RestKontroller {
	...
	public ResponseEntity<?> downloadJson() throws ClientProtocolException, IOException {
		...
		return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON).body(this.entitiesTable());
	}
	...
	public ResponseEntity<?> downloadJsonFile() throws ClientProtocolException, IOException {
		OpenAM.getInstance().download();
		OpenAM.getInstance().process();
		return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment;filename=openam.json")
				.contentType(MediaType.APPLICATION_JSON).body(this.entitiesTable());
	}
	...
	public ResponseEntity<?> reoport() throws ClientProtocolException, IOException {
		...
		result.put("COUNT_OAUTH", this.apps().filter(v -> EntityType.OAUTH2.equals(v.getEntityType())).count());

		result.put("COUNT_SAML", this.apps().filter(v -> (EntityType.SAML2.equals(v.getEntityType())
				&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER))).count());

		result.put("COUNT_WSFED", this.apps().filter(v -> (EntityType.WSFED.equals(v.getEntityType())
				&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER))).count());

		final Pattern patternCOT2031 = Pattern.compile("31|2031");

		result.put("COUNT_2025", this.apps()
				.filter(v -> ((EntityType.SAML2.equals(v.getEntityType()) || EntityType.WSFED.equals(v.getEntityType())) //
		...
		result.put("COUNT_2025_INTERNAL_ONLY", this.apps()
				.filter(v -> ((EntityType.SAML2.equals(v.getEntityType()) || EntityType.WSFED.equals(v.getEntityType())) //
						&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
						&& v.hasAttribute(Entity.COT) && !patternCOT2031.matcher(v.getAttribute(Entity.COT)).find() //
						&& v.hasAttribute(Entity.EXTERNAL_AUTH) && "N/A".equals(v.getAttribute(Entity.EXTERNAL_AUTH))))
				.count());

		result.put("COUNT_SAML2_2025", this.apps().filter(v -> ((EntityType.SAML2.equals(v.getEntityType())) //
				&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
		...
		result.put("COUNT_SAML2_2025_INTERNAL_ONLY",
				this.apps().filter(v -> ((EntityType.SAML2.equals(v.getEntityType())) //
						&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
						&& v.hasAttribute(Entity.COT) && !patternCOT2031.matcher(v.getAttribute(Entity.COT)).find() //
						&& v.hasAttribute(Entity.EXTERNAL_AUTH) && "N/A".equals(v.getAttribute(Entity.EXTERNAL_AUTH))))
						.count());

		result.put("COUNT_WSFED_2025", this.apps().filter(v -> ((EntityType.WSFED.equals(v.getEntityType())) //
				&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
				&& v.hasAttribute(Entity.COT) && !patternCOT2031.matcher(v.getAttribute(Entity.COT)).find())).count());

		result.put("COUNT_WSFED_2025_INTERNAL_ONLY",
				this.apps().filter(v -> ((EntityType.WSFED.equals(v.getEntityType())) //
						&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
						&& v.hasAttribute(Entity.COT) && !patternCOT2031.matcher(v.getAttribute(Entity.COT)).find() //
						&& v.hasAttribute(Entity.EXTERNAL_AUTH) && "N/A".equals(v.getAttribute(Entity.EXTERNAL_AUTH))))
						.count());
		...
		result.put("COUNT_2031", this.apps()
				.filter(v -> ((EntityType.SAML2.equals(v.getEntityType()) || EntityType.WSFED.equals(v.getEntityType())) //
						&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
						&& v.hasAttribute(Entity.COT) && patternCOT2031.matcher(v.getAttribute(Entity.COT)).find()))
				.count());

		result.put("COUNT_2031_INTERNAL_ONLY", this.apps()
				.filter(v -> ((EntityType.SAML2.equals(v.getEntityType()) || EntityType.WSFED.equals(v.getEntityType())) //
						&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
						&& v.hasAttribute(Entity.COT) && patternCOT2031.matcher(v.getAttribute(Entity.COT)).find() //
						&& v.hasAttribute(Entity.EXTERNAL_AUTH) && "N/A".equals(v.getAttribute(Entity.EXTERNAL_AUTH))))
		...
		result.put("COUNT_SAML2_2031", this.apps().filter(v -> ((EntityType.SAML2.equals(v.getEntityType())) //
				&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
				&& v.hasAttribute(Entity.COT) && patternCOT2031.matcher(v.getAttribute(Entity.COT)).find())).count());
		...
		result.put("COUNT_WSFED_2031", this.apps().filter(v -> ((EntityType.WSFED.equals(v.getEntityType())) //
				&& v.getAttribute(Entity.SP_IDP).equals(Entity.SERVICE_PROVIDER) //
				&& v.hasAttribute(Entity.COT) && patternCOT2031.matcher(v.getAttribute(Entity.COT)).find())).count());
